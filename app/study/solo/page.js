...
import { FaChevronLeft, FaClock, FaImage, FaListUl, FaMusic, FaPause,
FaPlay, FaPlus, FaRedo, FaStepForward, FaTimes, FaTrash, FaCheck, FaEdit,
FaHome, FaVolumeUp, FaVolumeMute, FaYoutube } from "react-icons/fa";
import { getFormattedTime, getFormattedDate } from
"../../../helpers/format";
...
{/* Subject Selection for logged-in users */}
{user && (
<div>
...
Settings changed. Reset required to apply.
</span>
<button
...
{/* Background menu content - unchanged */}
{menuOpen === 'background' && (
<div className="">
...
<button
className={`px-6 py-2 rounded-full font-semibold transition-all shadow text-sm 
${bgTab === 'static' ? 'bg-green-500 text-white' : 'bg-white/60 text-green-900 hover:bg-green-100'}`}
...
<button
className={`px-6 py-2 rounded-full font-semibold transition-all shadow text-sm 
${bgTab === 'live' ? 'bg-green-500 text-white' : 'bg-white/60 text-green-900 hover:bg-green-100'}`}
...
${bg === opt.key && !youtubeBg ? "bg-yellow-500 text-white font-semibold" : "bg-green-500/10 text-green-900 hover:bg-green-100"}`}
>
{opt.label}
</button>
))}
...
onChange={(e) => setAmbientVolumes((v) => ({
...v,
...
{user && (
<div className="flex items-center gap-2">
<span className="text-blue-400 text-xs">Subject:</span>
<span className="text-blue-200 text-xs">{sessionSubject}</span>
</div>
)}
<div className="flex items-center gap-2">
{(() => {

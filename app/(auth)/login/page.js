...
export default function CustomLogin() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError(err.errors?.[0]?.longMessage || "Google login failed");
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <form onSubmit={handleLogin}>
            <div className="space-y-4">
              ...
              <button 
                type="submit"
                disabled={isLoading}
                className="w-full bg-green-600 hover:bg-green-600 text-white font-medium py-3 px-4 rounded-lg transition-colors shadow-md disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center"
              >
                {isLoading ? (
                  <>
